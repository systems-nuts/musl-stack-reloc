...
void _start_c(long *p)
{
	...
	Auxv *auxv; 
	int i, copied =-1, size, total_size =-1;
	 ...
	 */
	//if (0x800000000000 > (unsigned long) stack_ptr) 
	...
	/* align max address */
	max = (max & ~(STACK_PAGE_SIZE -1)) + STACK_PAGE_SIZE; // We checked at the linux kernel that the alignment is aligned at 0xf with no address randomization

	//max = 0x1000000000000; aarch64
	//max = 0xfffffffff000; x86-64
	
	size = (max - ((unsigned long)stack_ptr) ) + STACK_PAGE_SIZE;
	//size = (size & ~(STACK_PAGE_SIZE - 1)) + (stack_ptr & 0xfff);
	//size = (size & ~(STACK_PAGE_SIZE - 1));

//	char verror_2 [22];
//	memset(verror_2, '0', sizeof(unsigned long)*2 +1);
//	_itoa_b16(verror_2, (unsigned long) max);
//	__syscall(SYS_write, 2, verror_2, strlen(verror_2));
...
#warning mmap used
	...
	// TODO Maybe insert here padding

#if STACK_RELOC_USE_MMAP
#warning memcpy_nostack used
...
__retry_mremap:
	...
	if ( ((unsigned long) stack_addr) > -4096UL) {
		...
		i = 5; 
		goto _error; // Goes to error TODO
	}
#endif /* !STACK_RELOC_USE_MMAP */
	...
	/* ARCH stack switch */
	// TODO for x86
	arch_stack_switch(STACK_END_ADDR - vdso_size, (long int)size);
...
_error:
{
	char serror [] = "crt3.c: _start_c ERROR 0\n";
	char verror [22];
	serror[23] += i;
...
__retry_mremap_x86:
	...
	/* mmap protect upper area */
	//__syscall(SYS_mmap, STACK_END_ADDR, arch_vaddr_max() - STACK_END_ADDR, 0, (MAP_PRIVATE|MAP_ANON|MAP_FIXED), -1, 0); TODO ask Antonio
...
#if STACK_RELOC_USE_MMAP
	/* unmap previous stack */
	//__syscall(SYS_munmap, (max - total_size), total_size); TODO ask Antonio
#endif /* STACK_RELOC_USE_MMAP */
